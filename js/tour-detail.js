...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function injectPromoBanner() {
        if (!isPromoMonth) return;
        const banner = document.createElement('div');
        banner.className = 'promo-banner';
        banner.innerHTML = `
            <div class="promo-banner-inner">
                <div class="promo-left">
                    <span class="promo-tag"><i class="fas fa-gift"></i> THÁNG THIẾU NHI</span>
                    <h2 class="promo-title">Trẻ em giảm tới <em>60%</em></h2>
                    <p class="promo-sub">Áp dụng cho tất cả các tour trong tháng 6 — Mang cả gia đình đi thôi nào! 🎉</p>
                </div>
                <div class="promo-right">
                    <div class="promo-badge-circle">
                        <span class="promo-pct">-60%</span>
                        <span class="promo-for">cho trẻ em</span>
                    </div>
                </div>
            </div>
        `;
        // Chèn banner ngay sau breadcrumb
        const breadcrumb = document.querySelector('.breadcrumb-nav');
        if (breadcrumb) breadcrumb.insertAdjacentElement('afterend', banner);
        else document.querySelector('main').insertAdjacentElement('beforebegin', banner);
    }
    ...
    function renderTourDetails(tour) {
        ...
        document.getElementById('tour-departure').innerHTML = `<i class="fas fa-paper-plane"></i> Khởi hành: ${tour.departure}`;
        ...
        // Tab Tổng quan
        let hlHtml = '<ul class="highlights-list">';
        tour.highlights.forEach(h => {
            hlHtml += `<li><i class="fas fa-check-circle"></i> ${h}</li>`;
        });
        hlHtml += '</ul>';
        document.getElementById('overview-content').innerHTML = hlHtml;
    ...
    document.getElementById('booking-sidebar-form').addEventListener('submit', e => {
        ...
        const mealPlan  = document.querySelector('.meal-plan-card.active')?.dataset.plan || 'none';
